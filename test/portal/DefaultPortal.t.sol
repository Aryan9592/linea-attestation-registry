...
contract DefaultPortalTest is Test {
  ...
  function test_attest_ModulePayloadMismatch(AttestationPayload memory attestationPayload) public {
    bytes[] memory validationPayload = new bytes[](2);
    vm.expectRevert(AbstractPortal.ModulePayloadMismatch.selector);
    defaultPortal.attest(attestationPayload, validationPayload);
  }
